      ...
      */
      ...
      return function(scope, iElement, iAttrs, controller) {
        ...
        var carouselId = 'rn-carousel-' + carousels,
            ...
            skipAnimation = true,
            initialLoad = true,             // bool to stop animation on page load
            initialLoadCounter = 0;         // deal with angular's 2X run through for setup
        ...
        function transitionEndCallback(event) {
         ...
         // console.log('transitionEndCallback', this, event);
          ...
          scope.$parent.loadPropertyFromPosition(scope.carouselCollection.position);
//          carousel.css('transform', '').css(translateSlideProperty(getTransformCoordinates(carousel[0]), false));
        ...
        // bind key events to navigate between listings
        $document.bind('keyup', function (evt) {
            // left arrow then right arrow
            if (evt.keyCode == 37) {
                scope.$apply(function () {
                    scope.carouselCollection.goTo(0, true);
                });
            } else if (evt.keyCode == 39) {
                scope.$apply(function () {
                    scope.carouselCollection.goTo(2, true);
                });
            }
        });
